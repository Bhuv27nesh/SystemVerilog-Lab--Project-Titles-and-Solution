...
package tb_pkg;
    ...
    class reference_model;
        ...
        function bit [3:0] get_expected(bit reset);
            if (reset)
                ref_val = 0;
            else
                ref_val = ref_val + 1;
    ...
    class scoreboard;
        ...
        function void check(bit [3:0] actual,
                            bit [3:0] expected,
                            int cycle,
                            bit clk,
                            bit reset);
            ...
            if (actual == expected)
                $display("%3d   | %0d   | %0d   | %01h      | %01h       | PASS",
                          cycle, clk, reset, actual, expected);
            else
                $display("%3d   | %0d   | %0d   | %01h      | %01h       | FAIL",
                          cycle, clk, reset, actual, expected);
